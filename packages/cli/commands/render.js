// `rig render <model> [--clip idle --time 0 --elevation 60 --heading 0] [-o out.svg]`
...
export function runRender(argv) {
  ...
  if (!target) { console.error('usage: rig render <model> [--clip --time --elevation --heading] [-o out.svg]'); return 2; }
  ...
  console.log(`rendered ${rig.id} (clip=${opts.clip ?? 'idle'} t=${opts.time ?? 0} elev=${opts.elevation ?? rig.camera?.elevation} heading=${opts.heading ?? rig.camera?.azimuth}) -> ${out}`);
  return 0;
}
